...
- 打包用 `com.gradleup.shadow` 做合并，`taboolibMainTask` 做注入、`packagePluginJar` 收口。**1.10.54 起最终 JAR【不再内嵌任何第三方库】**：gson / sqlite-jdbc / snakeyaml / TabooLib common-reflex 及其 Kotlin、ASM、commons-lang3 全部改为运行期由插件加载器下载（见「第三方运行期依赖（外置）」一节）。因此 `build.gradle.kts` 里【没有】任何 `relocate`，`embeddedLibraries` 刻意保持为空。
...
## 第三方运行期依赖（外置，1.10.54 起）

gson / sqlite-jdbc / snakeyaml / TabooLib common-reflex 及其 Kotlin、ASM、commons-lang3 **不在插件 JAR 里**，由 `linmumua.doudizhu.MuzPluginLoader`（`paper-plugin.yml` 的 `loader:` 声明）在插件类加载器建立【之前】下载。

- **只用 Paper 的 loader API，不用 `libraries` 键**：`paper-plugin.yml` 的 `libraries` 是旧 Bukkit `PluginLoader` 的机制，Paper 插件加载第三方库走实现 `io.papermc.paper.plugin.loader.PluginLoader` 并把类名写进 `loader:`。加载器里用 `MavenLibraryResolver.addDependency/addRepository` 登记坐标与仓库。
- **加载器只许依赖 JDK / Paper / Aether**：它被实例化时插件 JAR 与所有第三方都还不可见。允许的 import 前缀是 `java.`、`io.papermc.paper.plugin.loader.`、`io.papermc.paper.plugin.bootstrap.`、`org.eclipse.aether.`；有测试（`MuzPluginLoaderTest.loaderOnlyDependsOnJdkPaperAndAether`）扫 import 守这条。往这个类里加本插件类型或其它第三方 import 会在实服上以 `NoClassDefFoundError` 炸在插件加载阶段。
- **坐标版本一律钉死**：7 条坐标全部精确版本、无区间无 SNAPSHOT，且必须与 `build.gradle.kts` 的 `compileOnly` / `testImplementation` 逐字一致（有测试守护）。common-reflex 的 POM 没有任何依赖声明，所以 Kotlin / ASM / commons-lang3 必须显式列出；gson 的 `error_prone_annotations`、sqlite-jdbc 的 `slf4j-api`、kotlin-stdlib 的 `org.jetbrains:annotations` 由 Aether 按 POM 的 compile 作用域自动收集（因此根依赖作用域不能写成 `test`/`provided`）。
- **两个仓库都必须注册**（2026-09-25 用 HTTP 实测过）：`MavenLibraryResolver.MAVEN_CENTRAL_DEFAULT_MIRROR`（id `central`）提供 gson / sqlite-jdbc / snakeyaml / kotlin-stdlib / asm / commons-lang3；`https://repo.tabooproject.org/repository/releases/`（id `tabooproject`）提供 `io.izzel.taboolib:common-reflex`（它在 Central 上是 404）。**不得直连 `*.maven.org`**——官方文档明确要求走镜像，否则违反 Maven Central 条款并会被限流；也不要硬编码镜像域名（保留管理员用 `PAPER_DEFAULT_CENTRAL_REPOSITORY` / `org.bukkit.plugin.java.LibraryLoader.centralURL` 覆盖的能力）。加载器**不设置任何 User-Agent**。
- **必须禁止 relocate**：库按原始坐标下载，运行期加载到的就是 `org.yaml.snakeyaml` / `com.google.gson` / `org.sqlite` / `taboolib.library.reflex`。任何 `relocate` 都会把业务字节码改到 `linmumua.doudizhu.libs.*` 这个【下载包里不存在】的路径，运行期必然 `NoClassDefFoundError`。这条是本版本最容易被「顺手补回来」的地方。
- **首次联网与缓存边界**：加载器用 Paper 的 `MavenLibraryResolver`，其 `LocalRepository` 是相对路径 `libraries`，落点是**进程工作目录**，实服上即 `<服务端根>/libraries/`（与 Bukkit `libraries:` 同一目录）。**首次启动必须联网**，JAR 进缓存后**重启不再下载**（Aether 对已存在的 release 工件跳过更新检查）。增量（新增一个包）同样需要联网。
- **失败即不启动**：加载器不吞异常。离线首启、仓库不可达、校验和不符（`checksumPolicy=fail`）都会让解析抛 `LibraryLoadingException`，Paper 因此判定插件加载失败。这是刻意选择：让「库缺失」在启动阶段明确报错，而不是等某个功能用到时再抛 `NoClassDefFoundError`。
- **测试依赖单独保留**：外置库在 `dependencies {}` 里只有 `compileOnly`（编译期）与 `testImplementation`（测试期）两种形态；**不得**再出现 `implementation`，否则会把库重新带进产物打包范围（`MuzPluginLoaderExternalizationContractTest` 守这条）。
- **MySQL 不在外置清单里**：`mysql-connector-j` 仍是 `compileOnly`，不进加载器坐标，本次未扩展 MySQL 范围。
- **验证手段**：产物侧由 `verifyRelocatedSnakeYaml` 与 `build/muz-release-audit.py` 双向校验（loader 声明与类、外置包一个不留、业务引用仍是原包名）；行为侧由 `muzExternalDependencyProbe` 任务真实解析并加载 SnakeYAML/Gson/sqlite/reflex。**这两者都不等于实服验证**：真实服务端的首次启动、离线首启的失败表现仍需进服确认。
- **如实记录的风险**：① 加载器与探针证据都来自本机；② 缓存位于服务端根目录，若该目录被清空或换机器需重新联网；③ 依赖由第三方 Maven 仓库提供，仓库若下线或改版会导致启动失败（这是外置方案固有的可用性耦合）；④ 管理员若用环境变量把 Central 镜像指到不可达地址，会表现为插件加载失败。
...
- 版本号写在 `build.gradle.kts` 的 `version`，当前源码版本为 `1.10.57`（**1.10.57 改动（本轮，2026-09-26）**：按用户提供的 1.10.55 实服日志做两项**最小**修复，均只碰交互提示路径，**未改**对局规则、owner 路由、资源管线或关闭路径。① **确认** 1.10.56 的实体 region lane **已经**把「实体不存在」改为静默：1.10.55 日志里那条按实体限频刷屏的 `跳过跨 region 的实体可见性操作 showEntity（实体不存在）` 出自改造前那版 `reportCrossRegionSkip`（`entity == null` → `reason = "实体不存在"`，与「跨 region」共用同一条告警），而 1.10.56 起改为 `EntityRegionLane` 投递后，`Submission.ENTITY_ABSENT`、执行时 `!entity.isValid()` 与玩家离线**三者都不留痕**，只有四类异常留痕（实体中途换 region、投递被关闭期拒绝、投递接缝抛错、玩家可见性 API 抛错，按实体 30 秒限频）；该语义已由行为测试 `PlayerOutputDispatcherEntitySkipBehaviorTest.实体不存在时不投递且不留痕` 锁定，本轮复核**未发现残留的「实体不存在」告警路径**（全主源码已无该字符串），因此**未做改动**。② 新增业务拒绝类型 `linmumua.doudizhu.game.InteractionRejectionException`（继承 `IllegalStateException`），把**金币/筹码门槛不足**这类「插件主动拒绝玩家操作、且文本本就是给玩家看的中文提示」的异常与系统故障分类开：`GameTable.addPlayer` / `toggleReady` / `ensureSeatEntryEligibility` 三个资格抛点与 `PhysicalTableManager.joinSeat` 的换桌门槛改抛该类型；`PhysicalTableManager.reportInteractionFailure` 增加**唯一**分类点——`instanceof InteractionRejectionException` 时直接返回（不记日志），玩家提示由调用方既有的 `hint(..., exception.getMessage(), RED)` 照旧发出，真正异常（调度拒绝、反射失败、IO 错误等）继续按 WARNING 记录**完整堆栈**并按「动作 + 异常摘要」限频。判据刻意用**异常类型而非消息文本**：文本会随文案漂移，也会误伤同样抛消息的真实故障（例如 Hotbar 布局算式不闭合那种内部不一致必须继续留栈）。其余规则类拒绝（座位已有人、阶段不符等）**有意保持** `IllegalStateException` 并继续留痕——那是 1.10.51 批次的既有契约（`InteractionFailureLoggingBehaviorTest` 明确断言 PASS_TURN/READY 的阶段拒绝必须留痕），本轮只把用户点名的「金币门槛」纳入新类型，**未扩大适用范围**。新增行为测试 `InteractionFailureLoggingBehaviorTest.金币门槛业务拒绝只提示不记堆栈而其它异常仍留痕`（用**同一个 JOIN 按钮**制造两种结果：门槛房抛业务拒绝 → 0 条日志；未放置桌抛未归类 `IllegalStateException` → 1 条带堆栈日志）与源码契约 `InteractionFailureLoggingContractTest.门槛不足必须一律走业务拒绝类型而不是裸IllegalStateException`（钉住四个抛点都标了类型 + 分类点按类型判定 + 抛点不得退回裸 `IllegalStateException`；行为测试只能同时驱动其中一条路径，漏掉的开局资格复核抛点由这条契约覆盖）。做 **mutation 反证三次**：临时禁用分类 → 行为用例变红并复现原刷屏文本；把 `GameTable.addPlayer` 抛点退回裸 `IllegalStateException` → 行为用例变红；把 `GameTable.ensureSeatEntryEligibility` 抛点退回裸 `IllegalStateException` → **只有契约用例变红**（"expected: <3> but was: <2>"），证明契约测试确实覆盖了行为测试到不了的抛点；三次恢复后均回绿。验证：`paper-26.2` `compileJava compileTestJava processResources processTestResources --rerun-tasks` 成功；定向 5 类 **26/26**（含实体 lane 两类与交互留痕两类）；全量 JUnit **1260/1260、187 个容器**，0 失败/跳过/中止（比 1.10.56 的 1258 多 2 项 = 新增 2 个测试方法）。**未验证（如实）**：本轮**未打包、未部署、未重启测试端**，没有任何实服运行期证据；「点 JOIN 不再刷堆栈」由本机 JUnit + 源码契约支撑，需实服重启后点一次 JOIN 确认。1.10.56 改动：修复 `PlayerOutputDispatcher.showEntity` / `hideEntity` 的**跨 region 可见性缺陷**——这两个入口以前把可见性操作投在**玩家自己的 lane** 上，而 Paper 的 `CraftPlayer#showEntity0/hideEntity0` 会经 `CraftEntity#getHandle()` 读实体状态（Folia 上该方法带 owner region 门禁，抛 `Accessing entity state off owning region's thread`），只要玩家与实体不在同一个 region 就必然落进跳过分支（实服日志：同一玩家对 15 个桌面实体的 `showEntity` 全部被跳过，并按实体限频刷告警）。现新增可注入接缝 `EntityRegionLane`，生产实现经 `MuzScheduler.runEntity` 把操作投递到**实体所属 region**：该 lane 内实体侧访问合法，玩家侧只做「裸字段读 + 发一条携带实体状态的包」（`CraftPlayer#getHandle()` 无门禁、`ServerGamePacketListenerImpl#send` 无 tick thread 门禁、`ChunkMap$TrackedEntity#updatePlayer` 只做 `AsyncCatcher.catchOp`），与 Folia 自己发实体包的位置一致；隐藏/显示状态本就按「玩家 × 实体」记在 `CraftPlayer#invertedVisibilityEntities`、由实体追踪的 `canSee` 生效，因此对跨 region 玩家同样有效。执行 lane 内顺序固定为「判实体有效性 → 复核 `Bukkit.isOwnedByCurrentRegion(entity)` → 按 UUID 解析当前在线玩家 → 调用玩家 API」，并以 try/catch 保证玩家 API 异常**不抛回**调用方（owner tick / refresh）；**只有**四类异常留痕（实体中途换 region、投递被关闭期拒绝、投递接缝抛错、玩家可见性 API 抛错，按实体 30 秒限频），实体不存在与玩家离线**不再告警**——这是「降低重复警告」的正解（消除成因，而不是压噪声）。1.10.55 改动：`CraftEngineFurnitureService` 的 CE 家具反射兼容性修复——方法查找恢复「父类/接口也查找」的继承链语义（`getMethodByTypes(..., false, false, …)` → `true`；CE 26.8 的 `furnitureById(Key)` 声明在父类 `AbstractFurnitureManager` 与接口 `FurnitureManager` 上，原先只认本类声明会把它误判为缺失，属行为回归），并把 TabooLib 抛出的受检 `NoSuchMethodException` 统一转成带「期望签名 + 实际同名方法 + 实际方法表」的运行时异常，使「CE 缺 API」在 `onEnable` 阶段降级留日志而不是让插件加载失败；`furnitureById` 缺失只降级牌型探测。新增 `CraftEngineFurnitureReflectionCompatibilityTest`（7 项）与两次 mutation 反证。**纯内部缺陷、无用户可见行为变化，无实服运行期证据**。**本版 Release 不单独发布资源包与 CraftEngine bundle**（资源已内置/无需单独发布），构建仍可生成用于校验，发布只上传三个目标 JAR。1.10.54 改动：按用户明确要求把全部第三方运行期库改为**运行期下载**——gson / sqlite-jdbc / snakeyaml / TabooLib common-reflex 及其 Kotlin、ASM、commons-lang3 全部从最终 JAR 移出，改由新类 `linmumua.doudizhu.MuzPluginLoader`（Paper `PluginLoader`，`paper-plugin.yml` 的 `loader:` 声明）在插件类加载器建立【之前】经 Paper 的 `MavenLibraryResolver` 下载缓存并加入 classpath。配套改动：① `build.gradle.kts` 里四个库由 `implementation` 改 `compileOnly`、`embeddedLibraries` 清空、shadow 与 taboolib 两处 `relocate` 全部删除（库按原始坐标下载，改名会让业务字节引用指向不存在的包路径），`Multi-Release` 属性随之移除（JAR 已无多版本条目）；② `verifyRelocatedSnakeYaml` 的断言方向整体反转为「外置边界校验」：校验 loader 声明与类一致、13 组外置包前缀一个都不许在 JAR 里、四个业务类的字节码仍引用原始包名、`taboolibMainTask` 指纹存在；③ `build/muz-release-audit.py` 同步反转（版本升 `1.10.54`、新增 loader 声明/类与业务引用断言）；④ 新增 `MuzPluginLoaderTest`（11 项：坐标集合、版本钉死、反射读生产解析器实际登记的依赖与仓库、Central 必须走 `MAVEN_CENTRAL_DEFAULT_MIRROR` 而非直连 maven.org、TabooLib 仓库必在、作用域不得是 test/provided、loader 只依赖 JDK/Paper/Aether、不得设 User-Agent）、`MuzPluginLoaderExternalizationContractTest`（4 项源码契约）、`PaperPluginDescriptorTest` 新增 loader 声明项；⑤ 新增真实的依赖解析/缓存/加载探针 `MuzExternalDependencyProbe`（Gradle 任务 `muzExternalDependencyProbe`，需联网、不挂 `build`）。**未做**：未部署、未重启服务端，**没有任何实服运行期证据**；首次启动联网、缓存离线边界、下载失败导致的加载失败都只由探针 + 编译 + 单测支撑，必须由实服启动验证。**不改** `CLAUDE.md`（按要求）。**MySQL 范围未扩展**（`mysql-connector-j` 仍 `compileOnly`、仍不进下载清单）。旧 SnakeYAML 内嵌重定位规则【被本次全部外置明确取代】。1.10.53 改动：修复「插件被禁用后 /muz 仍可被派发，派发去解析插件类加载器里的类、而 Paper 已关闭插件 JAR，抛 `java.util.zip.ZipException: zip file closed`」。三处改动均在命令注册与关闭路径，未触碰对局规则与 owner 路由：① `DoudizhuPlugin` 新增 `private volatile Command muzCommand` 保存已注册命令引用，`registerMuzCommand()` 改为幂等——注册前先摘掉上一份引用、注册失败（同名命令被别的插件占用）不记为自身命令并告警，避免重复注册泄漏与「关闭时摘错对象」；② `onDisable` 新增 `runShutdownStep("注销 /muz 命令", this::unregisterMuzCommand)`，排在其它拆解步骤**之前**，确保注销发生在 Paper 关闭插件 JAR 之前，注销失败只记日志（`detachMuzCommand` 两处 catch `RuntimeException | Error`）、不抛异常、不打断关闭；摘除分两步，`Command.unregister(commandMap)` 只清命令自身引用，真正决定「能否被派发」的是 `CommandMap.getKnownCommands()`——Paper 的 `CraftCommandMap` 用的是转发到 Brigadier dispatcher 的 `BukkitBrigForwardingMap`（已用 javap 核对 `remove(key)` → `dispatcher.getRoot().removeCommand(key)`），按 key 摘除会同步删掉 Brigadier 节点；③ `DoudizhuCommand.onCommand`/`onTabComplete` 顶部新增 `isRuntimeAvailable()` 门禁（`plugin.isEnabled() && !plugin.isShuttingDown()`，并判空 plugin），门禁只用服务端类路径的 `Component.text(...)` 提示、**刻意不调用 `message()`/`MuzTheme`**，否则该兜底路径自身就会解析本插件类。新增行为测试 `MuzCommandLifecycleBehaviorTest`（10 项）与源码契约 `MuzCommandShutdownWiringContractTest`（7 项），并做 mutation 反证（临时禁用门禁与 knownCommands 摘除后 4 条用例变红，恢复后 17/17 通过）；`build/muz-release-audit.py` 的版本断言随之更新为 `1.10.53`；未打包、未部署、未重启测试端，无实服运行期证据）。1.10.52 改动：按用户明确要求把 `paper-plugin.yml` 的 `folia-supported` 由迁移期的 `false` **正式改为 `true`**——这是正式声明支持 Folia，不再只是冒烟临时值；同步升版本、更新 AGENTS.md / README.md，并把守护测试 `FoliaDeclarationMatchesSchedulerTest` 的声明断言由「必须为 `false`」改为「必须为 `true`」（其余 owner 路由断言一律未动）。1.10.51 改动：把原本只靠 `runtimeClasspath` 进包的 `gson:2.11.0` 与 `sqlite-jdbc:3.46.1.0` 显式并入 `embeddedLibraries` 并明确**保持原包名、不重定位**，同时在 `verifyRelocatedSnakeYaml` 与 `build/muz-release-audit.py` 各加一条「内嵌且原包名必须存在」的反向断言，把该决策固化为可执行契约；**1.10.51 的另一批改动（2026-09-24）为四项 Folia/区域化 lane 加固**：① `TableSpeechPanelService` 关闭期实体删除改为按 owner region 投递；② `PhysicalChipService` 增加玩家 region 归属门禁、跨 region 时拒绝写入；③ `TableGadgetService` 与 `TableGadgetBarHudService` 的 global 扫描异常隔离并限频留痕；④ `PhysicalTableManager` 交互异常改为服务端限频日志（详见下方「本轮」记录）；此前 1.10.50 的内容为：实体筹码已改为按背包与副手实际物品计数；桌内道具改为九格图标额外栏，第九格为语音入口；本轮继续迁移 Folia 牌桌 owner，新增单桌 region 世界刷新与自有实体 owner PDC，并继续收口 GameTable、TrickHudService、PhysicalTableManager、世界桌/手牌 GUI 监听器和麻将广播的 player lane；本轮又把 `rebuildAllTables`/`rebuildSingleTable`/`shiftAllAnchors` 改为「3×3 footprint 异步预热 + 锚点 region 世界体 + global 收口」的顺序化 stage 流水线；本轮又让重建在收口成功前**保留旧放置快照与索引**（不再提前 clear/remove + 切回未放置）、把身份/关闭/放置代次闸门**前移到 spawn 之前**并对无法提交的新桌做锚点 region 清理、并把 warmup/post-restore 门禁改为不依赖瞬时 placedTableCount；本轮又把 TabooLib 实际接入构建与反射路径）。`CLAUDE.md` 的版本约定必须与此保持同步（**注意：本轮按用户要求未改 `CLAUDE.md`，其版本段仍停留在 `1.10.51` / `folia-supported: false`，属已知待同步项。**）。`paper-plugin.yml` 用 `${version}` 占位，不要手写。
  - **当前版本事实（勿与历史记录混用）**：版本 `1.10.57`；`folia-supported: true`（**正式声明支持 Folia**，2026-09-24 起生效；此前长期为 `false`，中途冒烟临时 `true` 已回滚过）；TabooLib **已实际接入**但**已不再是内嵌库**（构建插件照用，`common-reflex` 等改由 MuzPluginLoader 运行期下载）；**内嵌库集合为空**——gson / sqlite-jdbc / snakeyaml / TabooLib common-reflex 及其 Kotlin、ASM、commons-lang3 全部外置；加载器方案见「第三方运行期依赖（外置）」一节；**关闭清理仍未完成**（`onDisable` 内的 owner 清理注册会被 `MuzScheduler` 关闭闸门拒绝，不得宣称已生效）。1.10.57 只做两件事：① **复核确认**（无改动）1.10.56 的实体 region lane 已把「实体不存在」改为静默；② 新增 `InteractionRejectionException` 并把**金币/筹码门槛**这类业务拒绝与系统异常分类开（交互 catch 只提示不记堆栈，真异常仍限频留完整栈），未改对局规则、owner 路由与资源管线；1.10.56 只改 `PlayerOutputDispatcher` 的实体可见性 lane（改投实体 owner region + 四类异常限频留痕、两类正常结果静默），1.10.55 只做 `CraftEngineFurnitureService` 的 CE 家具反射兼容性修复（继承链查找 + 缺方法降级留日志 + `furnitureById` 按可选 API 处理，纯内部缺陷），1.10.54 只做「全部第三方运行期库外置 + 加载器 + 门禁反转 + 测试 + 文档」；三者都未改任何对局规则；真实 Folia 实服矩阵仍有未覆盖路径，见「Folia 支持状态」一节的残余风险清单。
...
运行期 YAML 一律走 `org.yaml:snakeyaml`，封装在 `MuzYamlConfig`。**禁止**用 Bukkit `YamlConfiguration` 作为运行期主方案，禁止引入 Jackson YAML。**1.10.54 起 SnakeYAML 不再内嵌重定位，改为运行期下载，但仍使用 `org.yaml.snakeyaml`，不要换解析器**；`verifyRelocatedSnakeYaml` 任务会校验「原始包名与旧重定位包都不在 JAR 里、且业务字节码引用的是原始包名」，不要绕过。
...
- `/muz` 命令注册与关闭期注销（`DoudizhuPlugin.registerMuzCommand` / `unregisterMuzCommand` / `detachMuzCommand` + `command.DoudizhuCommand`）：`paper-plugin.yml` 不支持 `commands` 段，`/muz` 只能在 `onEnable` 经 `CommandMap` 手动注册，并把实例存进 `muzCommand` 字段。**CommandMap 不会随插件 disable 自动摘除命令**，所以 `onDisable` 必须在 Paper 关闭插件 JAR 之前经 `runShutdownStep("注销 /muz 命令", …)` 把它注销——否则插件被禁用后残留的 `/muz` 仍会被派发，派发去解析插件类加载器里的类时 JAR 已 close，抛 `java.util.zip.ZipException: zip file closed`。摘除要分两步：`Command.unregister(commandMap)` 只清命令自身的注册引用，真正决定「能否被派发」的是 `CommandMap.getKnownCommands()`（Paper 的 `CraftCommandMap` 用转发到 Brigadier dispatcher 的 `BukkitBrigForwardingMap`，按 key `remove` 会同步 `dispatcher.getRoot().removeCommand(key)`），两步都要做。注销失败只记日志、不外抛，绝不打断关闭；注册幂等（重复注册先摘掉旧引用，避免泄漏与「关闭时摘错对象」），注册失败（同名命令被别的插件占用）不记为自身命令。`DoudizhuCommand.onCommand` / `onTabComplete` 顶部还有一道 `isRuntimeAvailable()`（`plugin.isEnabled() && !plugin.isShuttingDown()`）门禁作为第二道保险——`isEnabled()` 在 Paper 进入 `onDisable` 之前就已置 false，`isShuttingDown()` 覆盖「仍启用但已开始拆解」的窗口；该门禁只读生命周期状态、只用服务端类路径的 Adventure 组件，**不得调用 `message()`/`MuzTheme` 等本插件类**（那样兜底路径自身就会解析本插件类而炸）。
...
- **注销 `/muz` 命令**（`unregisterMuzCommand`，经 `runShutdownStep("注销 /muz 命令", …)`，排在其它拆解步骤之前）。`CommandMap` 不会随插件 disable 自动摘除命令，若不在 Paper 关闭插件 JAR 之前摘掉，残留命令被派发时会解析已关闭的插件类加载器并抛 `java.util.zip.ZipException: zip file closed`；摘除失败只记日志、不得打断关闭。详见「领域模型约束」中 `/muz` 命令注册与关闭期注销一条。

## Folia 支持状态

**【正式声明 Folia 支持（本轮，2026-09-24，1.10.52）——先读这段】**

按 linmumua 的明确要求，`src/main/resources/paper-plugin.yml` 的 `folia-supported` 已由迁移期的 `false` **正式改为 `true`**，这是**正式声明支持 Folia**，不再是冒烟临时值。本轮只做「声明 + 版本 + 文档 + 守护测试」四件事，**未改动任何 owner 路由逻辑**。

- 声明依据：`MuzScheduler` 已实际承载 global / region / entity(玩家) / async 四条 owner lane，牌桌、玩家、实体与麻将领域的 owner 路由均已收口到调度门面；相应契约测试（`FoliaRuntimeLaneContractTest`、`FoliaDeclarationMatchesSchedulerTest` 的调度接缝断言等）全绿；真实 Folia 服务端多轮冒烟已确认插件能加载、开局推进、且早期记录的 `No currently ticking region` / `teleportAsync while in region threading` / `Async chunk retrieval` 三类区域线程违规清零。
- 守护测试变更：`FoliaDeclarationMatchesSchedulerTest` 的两处**声明**断言方向反转——`declarationRemainsFalseUntilRealFoliaAcceptance` 改名为 `declarationIsTrueAndBackedBySchedulerAdapters` 并断言 `folia-supported == true`，`decisionCommentIsPreserved` 改为断言注释仍在且值为 `true`。**其余断言（业务层不得绕过 `MuzScheduler` 直调 Bukkit 调度器的全源码扫描等 owner 路由契约）一字未改，没有弱化。**
- **因此下方所有历史段落里「`folia-supported` 仍为临时 `true`」「保持 `false`」「守护测试按设计变红」「必须继续为 `false`」的表述均已失效**，仅作为历史轮次记录保留；「当前事实勘误（本轮，2026-09-24）」那段写的「已恢复为 `false`、守护测试应全绿」也已被本次正式声明取代。

**仍不支持「无风险」的残余项（声明为 true 不代表这些已完成，改动相关路径前必须继续按 Folia lane 约束办事）**：

1. **关闭清理仍未完成**：`MuzScheduler` 在 `shuttingDown` 置位后拒绝一切任务注册，而 `onDisable` 阶段该标志早已置位，因此牌桌/麻将的 owner 清理注册在关闭期**实际不会执行**，reload 关闭分支退化为空操作。这条与是否声明支持 Folia 无关，仍属未实现。
2. **真实 Folia 实服矩阵未全量覆盖**：关闭期清理、区块实际卸载/重载、remove/reload/shutdown 全量 `RegionTaskBarrier` 只有源码契约与编译证据，没有运行期证据；`rebuildAllTables` 等重建路径的 region 投递在真实 region 线程上的表现也未逐条实服验证。
3. **测试端环境差异**：现场 CraftEngine 版本与编译依赖不匹配（`CraftEngine.reloadPlugin` 的 `NoSuchMethodError`），HUD 资源恢复在该端无法应用——属环境问题，不是 lane 违规，但在该端上无法据此验收资源流程。
4. 声明为 true 后，Folia 的插件管理器会放行加载，管理员可能直接装到真实 Folia 上；因此**任何新增的世界/实体/玩家操作必须继续遵守 owner lane 规则**，不得依赖「反正不会在 Folia 上跑」的假设。

**【历史：当前事实勘误（2026-09-24，已被上段取代）】** 以下长段落包含大量历史轮次记录，其中多处写着「`folia-supported` 仍为临时 `true`」「守护测试按设计变红」——那些都是**当时**的冒烟临时状态，**现已全部失效**。该轮次记录的真实状态是：

- `src/main/resources/paper-plugin.yml` 的 `folia-supported` **曾恢复为 `false`**（临时冒烟状态已回滚）。
- 因此 `FoliaDeclarationMatchesSchedulerTest` 当时应当全绿：历史记录里那些「2 项失败为 `FoliaDeclarationMatchesSchedulerTest` 在临时 `folia-supported: true` 下按设计变红」不再适用。**（本轮更新：该键现已正式为 `true`，守护测试断言方向已随之反转，见上段。）**
...
**【本段末尾勘误（2026-09-24）】** 上面这条 2026-09-23 记录里写的「2 项为临时 `folia-supported: true` 下按设计变红的声明守护测试」只是**该轮次当时**的状态快照。此后冒烟结束、`folia-supported` 曾恢复为 `false`；**本轮（1.10.52）又按用户要求正式改为 `true`**，因此该段提到的测试红绿状态与当前断言方向均已不同。该段中的 1171 项 / 174 容器同样是历史快照，不是当前数字。当前事实见本节开头的「正式声明 Folia 支持」。原文保留不改，仅此追加说明。

【本轮：四项 Folia/区域化 lane 加固（2026-09-24）】本轮四项修复均为源码 + 测试级证据，**没有实服运行期验证**；本轮只做本地编译与 JUnit，**未打包、未部署、未重启测试端**；`folia-supported` 保持 `false`。(1) `TableSpeechPanelService` 关闭期实体删除改为 owner region 投递。此前 `removePanels` 直接 `view.display.remove()`，而清理入口 `clearPlayer`/`clearTable`/`clearAll`/`shutdown` 可能跑在玩家 lane（玩家已走远）、桌 owner lane 或 `onDisable` 的主线程上，错误 lane 上 `remove` 抛 `Accessing entity state off owning region's thread` 并沿 `onDisable` 链路外溢。现新增可注入接缝 `PanelEntityLane`（`isOwnedByCurrentRegion`/`removeNow`/`removeOnOwnerRegion`，与 `PhysicalTableManager.WorldBodyLane`、`TableGadgetEffectService.EffectRuntime` 同形）；`removePanels` 逐张走 `removePanelEntity`，后者先判归属、本 region 直接删、否则把删除投回实体所属 region，投不出去（世界卸载、调度拒绝）留痕（`reportFailure("语音面板实体删除失败", …)`）；关服分支（`isStopping()`）只清追踪、不碰实体；`shutdown()` 先 `stopped = true` 再 `clearAll()`（先封入口，避免清理异常让服务既不关闭也不清空）。(2) `PhysicalChipService` 增加玩家 region 归属门禁。区域化核心的 `Bukkit.isPrimaryThread()` 实为 `TickThread.isTickThread()`，region 线程与 global 线程都为 true；而 `CraftInventory`/`PlayerInventory` 的写入路径没有任何服务端归属门禁（不像 `entity.remove()`/`showEntity` 会抛异常），所以「是 tick 线程」不等于「持有这个玩家的 region」，旧行为是**静默**跨 region 写入。现新增 `requireOwnerLane(playerId, player)`：在 `requirePrimaryThread()` 之外再查 `runtime.isOwnedByCurrentRegion(player)`，不持有该玩家 region 就抛 `IllegalStateException` 拒绝写入；生产适配器 `RuntimeAccess.bukkit()` 用 `Bukkit.isOwnedByCurrentRegion(Player)`，认不出的 handle 一律按「不拥有」处理（安全侧）。批量转移改为逐人预检，任一人不在本 lane region 内则整批拒绝、不触碰任何背包。(3) `TableGadgetService` 与 `TableGadgetBarHudService` global 扫描异常隔离与限频留痕。硬约束来自 `MuzScheduler.schedule`：周期回调抛异常会走 `managed.fail(repeating=true)` 并取消后端句柄，一次扫描失败升级为整条周期任务永久取消。`TableGadgetService.tick()` 现整段兜底（快照获取 `tables()` 也在兜底 try 内，`getTables()` 内部 `new ArrayList<>(tables.values())` 在并发结构修改下本身可能抛 CME/越界），逐桌保留 `catch (IllegalArgumentException ignored)` 静默跳过已注销桌、`catch (RuntimeException)` 限频记录；新增 `reportFailure(String, Throwable)`（10 秒窗口 `FAILURE_LOG_INTERVAL_MILLIS`）。`TableGadgetBarHudService.prefetchActivePlayers()` 整段兜底 + manager 空值早退 + `reportPrefetchFailure` 限频；`redrawHud` 投递前检查牌桌仍注册并捕获 `RuntimeException`（不止 `IllegalArgumentException`），失败经 `reportRedrawSkipped` 按「桌 + 玩家」限频。两处都不再让异常逃到调度器，也不静默吞掉。(4) `PhysicalTableManager` 交互异常服务端限频日志。此前 `handleInteraction` 及同源点击 catch（手牌选牌 `toggleHandCardSelection`、手牌出牌 `playSelectedHandCard`）把任意 `RuntimeException` 只转成一条玩家 ActionBar 提示、**不写服务端日志**，使「卡在正在发牌」这类调度失败在服务端完全不可见（玩家只看到一句提示、控制台没有任何堆栈）。现新增 `reportInteractionFailure(action, player, exception)`，键为「动作 + 异常摘要」、10 秒窗口（`INTERACTION_FAILURE_LOG_INTERVAL_MILLIS`），服务端 WARNING 带完整异常本体；玩家提示与对局流程完全不受影响。验证（全部真实执行）：`paper-26.2` 强制 `compileJava compileTestJava` 成功；本轮四项修复的定向 JUnit **46/46 通过**（8 容器，0 失败/跳过/中止，含 `PhysicalChipServiceTest`、`TableGadgetGlobalSweepSurvivalBehaviorTest`、`TableSpeechPanelServiceCleanupBehaviorTest`、`InteractionFailureLoggingBehaviorTest`、`InteractionFailureLoggingContractTest`、`FoliaRuntimeLaneContractTest`、`TableGadgetBarHudRedrawRegistrationTest`）；`paper-26.2` 全量 JUnit **1209 项 / 181 容器 / 1209 通过 / 0 失败**，其中 `FoliaDeclarationMatchesSchedulerTest` **已全绿**（`folia-supported: false` 的应然结果）。新增测试：`TableSpeechPanelServiceCleanupBehaviorTest`（8 项，覆盖错误 lane 必须把删除投回实体 region、owner lane 直接删、关桌清理、投递失败留痕不吞异常、关服期不触碰实体、shutdown 先封入口且幂等）；`TableGadgetGlobalSweepSurvivalBehaviorTest`（5 项，覆盖快照失败/收尾投递失败不打死周期任务、manager 缺失安静早退、九格栏预取失败限频留痕）；`InteractionFailureLoggingBehaviorTest` + `InteractionFailureLoggingContractTest`（覆盖按钮/选牌/出牌三处 catch 都留痕且限频、生产 helper 形状）；`PhysicalChipServiceTest` 新增 3 项（区域化 tick 线程不持有玩家 region 被拒且无变更、批量转移逐人预检、生产适配器真的调 `Bukkit.isOwnedByCurrentRegion`）；`FoliaRuntimeLaneContractTest` 新增 3 项契约（`speechPanelRemovalIsLaneGated`、`speechPanelShutdownClosesEntryBeforeCleanup`、`globalSweepPeriodicTasksAreExceptionGuarded`）。**未验证（如实）**：本轮只有编译 + JUnit 证据，**没有**真实 Folia / Leaf 运行期验证；`isOwnedByCurrentRegion` 在真实 region 线程上对语音面板删除投递、实体筹码 region 门禁、两处 global 扫描兜底与交互异常限频的实际效果仍需实服确认。

【本轮：正式声明支持 Folia（2026-09-24，1.10.52）】按 linmumua 明确要求，把 `paper-plugin.yml` 的 `folia-supported` 由迁移期的 `false` 正式改为 `true`，并同步升版本与文档。改动面严格限定为四项：(1) [src/main/resources/paper-plugin.yml](src/main/resources/paper-plugin.yml) 的声明键改为 `true`，注释由「迁移中必须保持 false」改写为**正式声明**，同时保留历史沿革说明与残余风险（关闭清理未实现、实服矩阵未全覆盖、测试端 CE 版本不匹配），并刻意保留「真实 Folia 服务端」字样供守护测试沿用。(2) `build.gradle.kts` 的 `version` 由 `1.10.51` 改为 `1.10.52`。(3) [src/test/java/linmumua/doudizhu/resources/FoliaDeclarationMatchesSchedulerTest.java](src/test/java/linmumua/doudizhu/resources/FoliaDeclarationMatchesSchedulerTest.java) 的两处**声明**断言方向反转：`declarationRemainsFalseUntilRealFoliaAcceptance` → `declarationIsTrueAndBackedBySchedulerAdapters`（断言 `folia-supported == true`，三个调度接缝断言原样保留），`decisionCommentIsPreserved` 的 `assertFalse(contains("folia-supported: true"))` → `assertTrue(...)`，其余业务层「不得绕过 `MuzScheduler` 直调 Bukkit 调度器」的全源码扫描断言**一字未改**（不弱化）。`declarationKeyExistsAndIsBoolean` 与两个私有 helper 未动。(4) AGENTS.md / README.md 同步本段与版本事实，历史轮次记录一律保留。**未改任何 owner 路由逻辑**，因此本轮不新增、也不减少任何 lane 契约。验证与产物结果见下方「本轮验证（1.10.52）」段。**如实说明**：`CLAUDE.md` 按用户要求未改，其版本段仍为 `1.10.51` / `folia-supported: false`，属已知待同步项；`build/muz-release-audit.py` 的 folia 断言随声明方向同步改为 `true`，版本匹配改为 `1.10.52`。

【本轮：/muz 命令关闭期生命周期修复（2026-09-24，1.10.53）】**这不是 Folia lane 改动**，而是「插件被禁用后残留命令仍被派发，触发解析已关闭的插件类加载器，抛 `java.util.zip.ZipException: zip file closed`」的实际修复；未触碰任何 owner 路由、对局规则或 region 投递逻辑，因此不新增/减少任何 lane 契约。已用 javap 核对 Paper 26.x API 作为实现依据（不是凭记忆）：`org.bukkit.command.CommandMap` 接口公开 `getKnownCommands()`；`Command.unregister(CommandMap)` 只做 `allowChangesFrom` 并清空命令自身引用，**不会**从 knownCommands 摘除；服务端 `CraftCommandMap extends SimpleCommandMap` 且其 `knownCommands` 传入的是 `io.papermc.paper.command.brigadier.bukkit.BukkitBrigForwardingMap.INSTANCE`（`extends HashMap<String, Command>`，覆写 `getDispatcher/remove/entrySet/…`），其 `remove(Object)` 实现为 `dispatcher.getRoot().removeCommand(key)`——**因此从 `getKnownCommands()` 按 key `remove` 会同步删掉 Brigadier 节点，这才是真正让 `/muz` 不再被派发的一步**。改动三处：(1) `DoudizhuPlugin` 新增 `private volatile Command muzCommand`；`registerMuzCommand()` 幂等（注册前 `detachMuzCommand(commandMap, previous, "重复注册")`，注册返回 false 时不记为自身命令并告警）。(2) `onDisable` 新增 `runShutdownStep("注销 /muz 命令", this::unregisterMuzCommand)`，排在「推进关闭代次」之后、其它拆解步骤之前；`unregisterMuzCommand()` 先清字段再摘除（幂等，重复调用安全），`detachMuzCommand` 分两步（`Command.unregister` + 按 key 摘 `getKnownCommands()`）且两处各自 catch `RuntimeException | Error` 并留痕，绝不外抛打断关闭。(3) `DoudizhuCommand.onCommand`/`onTabComplete` 顶部新增 `isRuntimeAvailable()`（`plugin != null && plugin.isEnabled() && !plugin.isShuttingDown()`）；门禁分支用 `Component.text(...)`（服务端类路径）而非 `message()`（会解析 `MuzTheme` 等本插件类）。新增 `MuzCommandLifecycleBehaviorTest`（10 项，Unsafe 夹具 + 复刻 `SimpleCommandMap` 可见语义的 `CommandMap` 替身 + 发送者/日志替身）与 `MuzCommandShutdownWiringContractTest`（7 项源码契约，含「注销步骤必须早于其它拆解步骤」这条无法在单测驱动的性质）；**mutation 反证**：临时把 `onCommand` 门禁置为不生效、并把 `known.remove(key)` 置为不执行后，4 条用例实跑变红（2 条以 NPE 命中 `getTableManager()` 证明进入了业务分支，2 条以「残留键 `[muz, muz:muz]`」「重复注册必须换成新实例」断言失败），恢复后 17/17 通过。`build/muz-release-audit.py` 的版本断言随之更新为 `1.10.53`。**未验证 / 剩余风险（如实）**：本轮**未打包、未部署、未重启测试端**，没有任何实服运行期证据；`BukkitBrigForwardingMap.remove` 删 Brigadier 节点的效果来自字节码核对而非实服行为，`isEnabled()` 在 Paper 进入 `onDisable` 之前即置 false 这一点同样来自 Paper 源码语义而非本轮实测；`muzCommand` 的注销时机正确性（确实早于插件 JAR 关闭）由 `onDisable` 内的位置保证，但「JAR 关闭」这一刻本身无法在单测里模拟。

【本轮：实体可见性改投实体 owner region —— `PlayerOutputDispatcher.showEntity/hideEntity` 跨 region 缺陷的真实修复（2026-09-25，1.10.56）】用户日志：同一玩家线程对 15 个桌面实体的 `showEntity` **全部**落进「跳过跨 region 的实体可见性操作」分支（按实体 30 秒限频仍刷出 15 条），即桌边动态/座位名/手牌的按人可见性在那批调用上一次都没生效。**根因由 javap 反编译测试端内核 `folia-26.1.2.jar` 落实（不是推断）**：① `org.bukkit.craftbukkit.entity.CraftEntity#getHandle()` 的方法体第一句就是 `TickThread.ensureTickThread(entity, "Accessing entity state off owning region's thread")`——**实体侧任何经 `getHandle()` 的读取都带 owner region 门禁**；② `CraftEntity#isVisibleByDefault()`（`getHandle().visibleByDefault`）、`CraftEntity#isTicking()`、`getTicksLived()` 都走它，而 `isValid()` / `isDead()` / `getUniqueId()` / `getWorld()` / `getLocation()` 是**裸字段读**（无门禁）；③ `CraftPlayer#showEntity0` 的步骤是「`getHandle().connection`（`CraftPlayer#getHandle()` 是裸字段读、**无门禁**）→ `entity.isVisibleByDefault()`（**有门禁**）→ `trackAndShowEntity`」，`hideEntity0` 同形（并额外要求 `plugin.isEnabled()`）；④ `ChunkMap$TrackedEntity#updatePlayer` 只有 `AsyncCatcher.catchOp("player tracker update")`（要求「是某个 tick 线程」，**不要求拥有该玩家**）、`ChunkMap$TrackedEntity` 本身无 tick-thread 门禁、`ServerGamePacketListenerImpl#send` 无门禁——**玩家侧那一半没有门禁，实体侧那一半有**；⑤ `TrackedEntity#updatePlayer` 会调 `CraftPlayer#canSee(entity)`，而 `canSee` 的实现是 `visibleByDefault ^ invertedVisibilityEntities.containsKey(uuid)`——**玩家的隐藏状态由实体追踪按实体 region 生效**，且该判断只在 `TickThread.isTickThreadFor(entity)` 为真时才做；⑥ `TrackedEntity#moonrise$removeNonTickThreadPlayers()` 会把「当前线程不是它的 tick thread」的玩家从 `seenBy` 里摘掉——**Folia 的实体追踪本身就把不同 region 的玩家排除在追踪集合之外**。因此结论有两条：**(a) 玩家 lane 是错 lane**（那里读实体状态必然抛，改造前的门禁只是把它吞成跳过）；**(b) 实体 owner region 是唯一既合法又生效的 lane**（实体侧访问全部合法；玩家侧只剩裸字段读与线程安全的发包；且与 Folia 自己发实体包的位置一致）。
**实现**：`PlayerOutputDispatcher` 新增可注入接缝 `EntityRegionLane`（`runOnEntityRegion(UUID, Consumer<Entity>) → Submission{SUBMITTED|ENTITY_ABSENT|REJECTED}`，`direct()` 仅供单测/已确认在实体本 region 的场景），生产实现 `schedulingEntityRegionLane(plugin)` 先在调用线程 `Bukkit.getEntity(uuid)`（`CraftServer#getEntity` → `Level#getEntity` 只是实体表查表，无门禁），再经 **`MuzScheduler.runEntity`**（→ Folia `EntityScheduler`，其内部只用 `CraftEntity#getHandleRaw()` 与 `TickThread#isTickThreadFor` 判定，同样不触发门禁，且实体被移除时会走 retired）投到实体所属 region；执行体 `applyVisibilityInEntityRegion` 的**固定顺序**是「`!entity.isValid()` 早退（投递后被移除属正常结果）→ `!Bukkit.isOwnedByCurrentRegion(entity)` 让位（实体中途换 region，记限频 WARN）→ `currentPlayer(viewerId)` 解析当前在线玩家（离线则静默返回）→ `action.accept(viewer, entity)`」，最后一层 try/catch 把玩家 API 异常转成限频 WARN，**绝不抛回**调用方（调用方是 owner tick / refresh，抛出去会打断整轮刷新）。该 lane **只允许**做这一件事：不得在这里读玩家实体状态或调用其它玩家 API，否则就是把「跨 region 访问」换个方向重犯。**告警策略（降低重复警告的正解）**：跨 region 现在会正常执行，所以原先那类告警的**成因被消除**；留痕只覆盖四类真正异常——实体中途换 region、投递被关闭期拒绝（`MuzScheduler` 关闭闸门返回已取消句柄，据此如实报 `REJECTED`）、投递接缝抛错、玩家可见性 API 抛错——按实体 30 秒限频并随下一次日志报出被压掉的次数；**实体不存在与玩家离线不再告警**（它们是正常结果，按玩家数重算会把告警刷满），`SKIP_LOG_INTERVAL_MILLIS` 常量与限频记账结构保留复用。
**测试与证据**：`PlayerOutputDispatcherEntitySkipBehaviorTest` 按机制变更整体升级为 10 项（新增：投递键必须是实体 UUID 且 **player lane 被使用次数恒为 0**、玩家离线不留痕、实体不存在不投递不留痕、投递被拒绝留痕、投递接缝抛错不外抛、玩家 API 抛错不外抛且留痕；保留并改到新路径的：归属成立时调用且不打日志、lane 内归属不成立时零调用 + 限频、窗口内被压次数随下一次日志报出、按实体 UUID 记账）；新增源码契约 `PlayerOutputDispatcherEntityRegionContractTest`（4 项：插件构造器必须装配调度版 lane 且不得用 `direct()`、实体 lane 必须走 `MuzScheduler.runEntity` 而非 `runRegion`/`runPlayer`、两个 UUID 入口不得出现 `runPlayer(`/`enqueuePlayer(`、执行体必须「有效性 → 归属 → 解析玩家 → 调用 API」且该顺序以字符串位置断言钉住、Folia 门禁原文必须留在注释里）；`PhysicalTableOwnerTickContractTest` 中**编码旧机制**的两条断言（「实体可见性必须在 player lane 通过 UUID 重新解析实体」「`entity != null && entity.isValid()`」）按机制升级为「必须经 `EntityRegionLane` 投递 + 执行 lane 内按 UUID 重新解析 + `!entity.isValid()` + `isOwnedByCurrentRegion` 复核」（机制变更，**不是弱化**）。**mutation 反证（全部实跑）**：① 让 `reportEntityLaneSkip` 直接早退（四类留痕全失效）→ 行为测试 **6/10 变红**（命中限频、投递被拒、投递异常、玩家 API 抛错、换实体记账、首次留痕）；② 把投递退回 player lane（用 `runPlayer(viewerId, player -> { })` 取代 `runOnEntityRegion`）→ **8/10 变红**，其中两条正是本次修复的核心断言（「两次调用都必须投递到实体 owner region」「实体归当前 region 时必须真实调用玩家可见性 API」）。恢复后两次均回到 14/14 绿。**实跑结果**：`paper-26.2` 强制 `compileJava compileTestJava processResources processTestResources --rerun-tasks` 成功（8 tasks executed；另两目标 `compileJava compileTestJava` 亦 BUILD SUCCESSFUL）；定向 81/81（12 类，覆盖上述 4 类与新契约）；全量 JUnit **1258/1258、187 个容器 / 0 失败 / 0 跳过 / 0 中止**（比 1.10.55 的 1248 多 10 项，即行为测试 4→10 与新增契约 4 项）。**本轮未认证部分（如实）**：只做本地编译 + JUnit，**未打包、未部署、未重启测试端、未进服**，没有任何实服运行期证据；「实体 region 内调用 `player.showEntity` 对跨 region 玩家是否真能让客户端看到」由上述字节码推导 + 单测支撑，仍需实服确认（尤其是 `updatePlayer` 的 `AsyncCatcher` 在真实 region 线程上的表现，以及 `PlayerShowEntityEvent`/`PlayerHideEntityEvent` 在**实体 region 线程**被触发对其它插件监听器的影响——MUZ 自身没有监听这两个事件，已核对）。**同族残留（本轮未动，如实记录）**：`TableSpeechPanelService#spawnPanel` 仍在「创建面板的那个 lane」里直调 `owner.showEntity(plugin, display)`（面板实体就地生成、与所有者同位置，故正常情形同 region；一旦两者不同 region 就会复现同类异常）；`TableGadgetEffectService#spawnDisplay` 用的是 `isOwnedByCurrentRegion(viewer)`（判**观众**归属）而非实体归属——因为实体是在该 lane 内刚生成的故当前无害；两条都是**同类缺陷的候选点**，改动前先按上面的 API 事实判断 lane，不要照搬「留在玩家 lane」的旧假设。

## 当前技术栈

| 层 | 技术 | 备注 |
|---|---|---|
| 平台 | Paper / Purpur / Folia，`folia-supported: true`（**正式声明**） | 目标 API 由 `MuzTarget` 表驱动，默认 `paper-26.2`；owner lane 走 `MuzScheduler` 四条接缝 |
...
| 配置 | `org.yaml:snakeyaml:2.6`（1.10.54 起**运行期下载**，不再内嵌重定位） | 封装于 `MuzYamlConfig`；仍用 `org.yaml.snakeyaml` |
| 存储 | `sqlite-jdbc:3.46.1.0`（1.10.54 起运行期下载），可选 `mysql-connector-j:8.4.0` | MySQL 仍为 `compileOnly` 且不在下载清单里 |
| 序列化 | `gson:2.11.0`（1.10.54 起运行期下载） | |
| 运行期依赖加载 | Paper `PluginLoader` + `MavenLibraryResolver`（`linmumua.doudizhu.MuzPluginLoader`，`paper-plugin.yml` 的 `loader:`） | 首次启动联网下载到 `<服务端根>/libraries/`；失败即插件加载失败；详见「第三方运行期依赖（外置）」 |
| 可选依赖 | CraftEngine 0.0.67、PlaceholderAPI 2.12.2、VaultAPI 1.7 | 全部 `compileOnly` |
| 构建 | Gradle + Kotlin DSL + `com.gradleup.shadow:9.3.0` + `io.izzel.taboolib:2.0.38` | 产物在 `build/<targetId>/`；shadow 合并、taboolibMainTask 注入、packagePluginJar 收口三段式；**1.10.54 起无 relocate、`embeddedLibraries` 为空** |
...
### 1.10.57（实体不存在已静默确认 + 金币门槛改按业务拒绝分类，本地验证通过，实服待验）

- 用户日志（MUZ 1.10.55）两项：① `showEntity` 告警「实体不存在」；② JOIN 按钮因金币不足抛 `IllegalStateException` 并打出完整 WARN 堆栈。
- ① **确认已修，未改动**：1.10.55 的 `PlayerOutputDispatcher` 用 `reportCrossRegionSkip(..., "实体不存在" | "跨 region")` 同一条告警覆盖「实体不存在」与「跨 region」两种情形（HEAD 版第 308 行 `reason = "实体不存在"`）。1.10.56 改为 `EntityRegionLane` 投递后，`Submission.ENTITY_ABSENT`（提交时 `Bukkit.getEntity` 为 null）、执行时 `!entity.isValid()`、玩家离线三者**都不留痕**；只有四类异常留痕（实体中途换 region、投递被关闭期拒绝、投递接缝抛错、玩家可见性 API 抛错，按实体 30 秒限频）。行为测试 `PlayerOutputDispatcherEntitySkipBehaviorTest.实体不存在时不投递且不留痕` 锁定该语义；本轮全主源码扫描确认已无「实体不存在」告警字符串，无残留路径。
- ② **修复**：新增 `linmumua.doudizhu.game.InteractionRejectionException`（继承 `IllegalStateException`，判据是**异常类型**而非消息文本）。改抛点 4 处——`GameTable.addPlayer`（加入）、`GameTable.toggleReady`（准备）、`GameTable.ensureSeatEntryEligibility`（开局资格复核）、`PhysicalTableManager.joinSeat`（换桌门槛），全部是 `insufficientEntryMessage` / `canAffordEntry` 家族。分类点**唯一**收口在 `PhysicalTableManager.reportInteractionFailure`：`instanceof InteractionRejectionException` → 直接返回、不记日志；玩家提示仍由 catch 里既有的 `hint(player.getUniqueId(), exception.getMessage(), NamedTextColor.RED)` 发出；其它异常继续 `plugin.getLogger().log(Level.WARNING, ..., exception)` 并限频。`InteractionRejectionException` 的 javadoc 写明「其余规则类拒绝仍用 `IllegalStateException`，不要顺手扩大适用范围」。
- 验证（全部真实执行）：`paper-26.2` `compileJava compileTestJava processResources processTestResources --rerun-tasks` 成功；定向 5 类 **26/26** 通过（`InteractionFailureLoggingBehaviorTest`、`InteractionFailureLoggingContractTest`、`PlayerOutputDispatcherEntitySkipBehaviorTest`、`PlayerOutputDispatcherEntityRegionContractTest`、`PhysicalTableOwnerTickContractTest`），全量 JUnit **1260/1260、187 个容器**、0 失败/跳过/中止；**mutation 反证**三次——禁用分类点 → 行为用例变红并复现「牌桌交互处理失败（按钮 JOIN）… InteractionRejectionException: 当前未挂钩经济系统，暂时不能参与低级场。」；把 `GameTable.addPlayer` 抛点退回裸 `IllegalStateException` → 行为用例同样变红（证明分类依赖抛点类型）；把 `GameTable.ensureSeatEntryEligibility` 抛点退回裸 `IllegalStateException` → **只有**契约用例变红（`expected: <3> but was: <2>`），证明契约覆盖了行为测试到不了的抛点；三次恢复后均回绿。
- **未验证（如实）**：未打包、未部署、未重启测试端，无实服运行期证据；实服需重启后点一次 JOIN（余额不足）确认控制台不再出现该堆栈。其余规则类拒绝（座位已有人、阶段不符）**仍会**记完整堆栈，属已知范围（见 1.10.51 批次契约），本轮未扩大。

### 1.10.56（实体可见性改投实体 owner region，本地验证通过，实服待验）

- 用户日志：同一玩家线程对 15 个桌面实体的 `showEntity` 全部落进「跳过跨 region」分支并按实体限频刷告警——开局后桌面实体的按人可见性（桌边动态、座位名/信息、手牌）一次都没生效。
- 根因（javap 反编译测试端 `folia-26.1.2.jar`，不是推断）：`CraftEntity#getHandle()` 带 `TickThread.ensureTickThread(entity, "Accessing entity state off owning region's thread")` 门禁，而 `Player#showEntity/hideEntity` 的实现（`CraftPlayer#showEntity0/hideEntity0`）会读 `Entity#isVisibleByDefault()`（走该门禁）并调 `TrackedEntity#updatePlayer`（内部按 `canSee` 决定是否把玩家加入 `seenBy`）。因此这两 API 只有在**实体所属 region** 上才合法；放在玩家自己的 lane 上，只要两者不同 region 就必然抛（改造前的 `isOwnedByCurrentRegion` 门禁把它吞成了静默跳过）。玩家侧那一半没有门禁：`CraftPlayer#getHandle()` 是裸字段读、`ServerGamePacketListenerImpl#send` 无 tick-thread 门禁、`ChunkMap$TrackedEntity#updatePlayer` 只有 `AsyncCatcher.catchOp`（要求「是某个 tick 线程」，不要求拥有玩家）。
- 实现：`PlayerOutputDispatcher` 新增可注入接缝 `EntityRegionLane`（返回 `SUBMITTED`/`ENTITY_ABSENT`/`REJECTED`，`direct()` 仅供单测），生产实现先在调用线程 `Bukkit.getEntity(uuid)`（无门禁的查表）再经 `MuzScheduler.runEntity` 投到实体所属 region；执行体顺序固定为「实体有效性 → `isOwnedByCurrentRegion(entity)` 复核 → 按 UUID 解析在线玩家 → 调玩家 API」，并以 try/catch 保证玩家 API 异常不外抛给 owner tick/refresh。该 lane 只允许做这一件事（不得读玩家实体状态或调其它玩家 API）。
- 告警策略：跨 region 的成因被消除后，留痕只覆盖四类异常（实体中途换 region、投递被关闭期拒绝、投递接缝抛错、玩家可见性 API 抛错），按实体 30 秒限频并报出被压次数；实体不存在与玩家离线改为静默（正常结果）。
- 验证：`paper-26.2` 强制 `compileJava compileTestJava processResources processTestResources --rerun-tasks` 成功（8 tasks executed，三目标 `compileJava compileTestJava` 亦均 BUILD SUCCESSFUL）；定向 81/81（12 类，含 `PlayerOutputDispatcherEntitySkipBehaviorTest` 10 项与新增契约 `PlayerOutputDispatcherEntityRegionContractTest` 4 项）；全量 JUnit **1258/1258、187 个容器**通过，0 失败/跳过/中止；**mutation 反证**——让 `reportEntityLaneSkip` 早退 → 6/10 变红，把投递退回 player lane → 8/10 变红（含两条核心断言），恢复后 14/14 绿。`PhysicalTableOwnerTickContractTest` 中两条编码旧机制的断言按机制升级（非弱化）。
- **未验证（如实）**：未打包、未部署、未重启测试端、未进服；「对跨 region 玩家是否真能看到」由字节码推导 + 单测支撑。**同族残留（未动）**：`TableSpeechPanelService#spawnPanel` 与 `TableGadgetEffectService#spawnDisplay` 仍在各自 lane 里直调 `owner.showEntity`（前者正常情形同 region；后者用 `isOwnedByCurrentRegion(viewer)` 判观众而非实体归属），改动前先按上述 API 事实判断 lane。

### 1.10.55（CE 家具反射兼容性修复，本地验证通过，实服待验）

- 修复（2026-09-25，本轮升为正式版本；纯内部缺陷、无用户可见行为变化）：`CraftEngineFurnitureService` 的 CE 家具反射兼容性。① **方法查找恢复继承链语义**：`methodByTypes` / `methodByTypesSilently` 原先传 `getMethodByTypes(name, false, false, …)`，第二个布尔量是「是否在父类/接口里继续找」，传 false 等于只认本类声明——实测 CE 26.8 的 `furnitureById(Key)` 声明在父类 `AbstractFurnitureManager` 与接口 `FurnitureManager` 上（子类 `BukkitFurnitureManager` 自己没有，已用 `javap` 核对运行期 jar），旧 `Class.getMethod` 会搜父类，所以这是一处**行为回归**；现传 `true`，与 `Class.getMethod` 同语义（先本类、再父类、再接口）。② **缺方法不再逃出 onEnable**：`ReflexClass.getMethodByTypes` 抛的是**受检** `NoSuchMethodException`，但 ReflexClass 是 Kotlin 类、字节码**没有 throws 子句**（javac 因此认为它不抛受检异常，直接 `catch (NoSuchMethodException)` 编译不过），于是它会从 `bridge()` 既有的 `catch (ClassNotFoundException | RuntimeException | LinkageError)` 与 hitbox 桥的 `catch (ClassNotFoundException | RuntimeException)` 旁边溜到 `onEnable`；现统一经 `instanceof` 判定转成**运行时异常**，并在消息里带上「期望签名 + 该类（含父类）实际同名方法 + 该类实际方法表」（诊断辅助 `describeMethodsNamed` / `describeAllMethods` / `signatureOf` / `parameterNames` 与 `CraftEngineOffsetService` 同一套写法），由调用点既有 catch 降级并写日志。③ **`furnitureById` 按可选 API 处理**：改用「找不到返回 null」的入口，缺失只降级 `detectPlacementKind`（该方法本就有 null 判空）并留一条带签名的告警，不再连带关掉家具放置/清理；其余 API 缺失仍使整条家具桥降级。④ 新增包内可见接缝 `ceClass(String, ClassLoader)`（生产就是一行 `Class.forName`，与 `CraftEngineOffsetService#fontManagerClass` 同源同理），**本类因此不再是 `final`**（与同包 `CraftEngineOffsetService` 一致，两者都为了「替身插件给不出自定义类加载器」这一步可测）；**不要顺手把它改回 `final`，否则新测试无法驱动生产 `bridge()`**。⑤ 新增 `CraftEngineFurnitureReflectionCompatibilityTest`（7 项：父类声明子类未声明、接口 default 声明、可选 API 彻底缺失时只降级并留期望/实际签名、必需 API 缺失时整桥降级且不抛出、hitbox 桥缺方法只降级 hitbox、缺方法被转成运行时异常而非受检异常、源码契约「查找沿继承链且不引入裸反射 API」），并做两次 **mutation 反证**：把两处 flag 改回 `false` → 3 项变红（父类/接口/契约）；删掉异常转换 → 3 项变红（必需缺失/hitbox/异常类型，且失败信息显示原始 `NoSuchMethodException` 从 `isAvailable()` 逃出、日志为空）。验证：`paper-26.2` 强制 `compileJava compileTestJava processResources processTestResources --rerun-tasks` 成功（8 tasks executed），定向 60/60（8 容器）与全量 JUnit **1248 项 / 186 容器 / 1248 通过 / 0 失败 / 0 跳过**（比 1.10.54 原始记录多出的 7 项/1 容器即新增测试类）。**未验证（如实）**：无任何实服运行期证据（未部署、未重启、未进服），「CE 26.8 上 `furnitureById` 现在真的能解析到」只由本机 `javap` 核对 + 替身夹具 + 单测支撑；其它两个目标本轮未编译（按要求只跑 `paper-26.2`）。同类查找的旁路核查（`javap` 实测）：`CraftEngineOffsetService` 与 `CraftEngineHudResourceBridge` 里也各有一处 `getMethodByTypes/getMethodByTypeSilently(..., false, false, ...)`，但前者分析的是**声明该方法的接口本身**（`FontManager.createMiniMessageOffsets(int)` 在 0.0.67 与 26.8 上都是该接口的 default 方法），所以 `false` 在那里无害；**后者 `findInHierarchy` 只沿 `Class.getSuperclass()` 逐层走、不含接口**，若 CE 把某个它要找的方法挪到接口上会同样误判为缺失——本轮未改，属已识别的残留风险。另：`javap` 核对 0.0.67（编译期依赖）也确认 `furnitureById` 声明在 `AbstractFurnitureManager` 上，故这处回归在编译目标版本上同样存在，不是 26.x 独有。

### 1.10.54（全部第三方运行期库外置，本地验证通过，实服待验）

- 需求（用户明确要求）：Gson、SQLite、SnakeYAML、TabooLib `common-reflex` 及其 Kotlin / ASM / commons-lang3 依赖**全部从最终 JAR 移出**，改为启动前经 Paper 的 `MavenLibraryResolver` 下载缓存。保留原生 `JavaPlugin` 与 TabooLib Gradle 插件，不引入注解生命周期。
- 实现：新增 [src/main/java/linmumua/doudizhu/MuzPluginLoader.java](src/main/java/linmumua/doudizhu/MuzPluginLoader.java)（`implements PluginLoader`），`paper-plugin.yml` 增加 `loader: linmumua.doudizhu.MuzPluginLoader`。API 与用法按官方文档核对（`PluginLoader.classloader(PluginClasspathBuilder)`、`MavenLibraryResolver.addDependency/addRepository`、`MavenLibraryResolver.MAVEN_CENTRAL_DEFAULT_MIRROR`），并用 `javap` 对 `paper-api 26.2` 逐条验证签名。该类只依赖 JDK / Paper loader API / Aether，**没有任何 relocate**。
- 坐标与仓库（2026-09-25 用 HTTP 实测）：Central 镜像提供 gson / sqlite-jdbc / snakeyaml / kotlin-stdlib / asm / commons-lang3；`https://repo.tabooproject.org/repository/releases/` 提供 `io.izzel.taboolib:common-reflex`（Central 上 404）。不直连 `*.maven.org`（走 Paper 镜像常量，保留管理员覆盖能力），不设置 User-Agent，版本全部钉死。
- 构建侧：四个库 `implementation`→`compileOnly`；`embeddedLibraries` 清空；shadow 与 `taboolib {}` 两处 `relocate` 删除；`Multi-Release` 属性删除；`testImplementation` 单独保留 7 条坐标（另加 `testCompileOnly` 两个 maven-resolver 实现类，供探针自建离线会话）；`mysql-connector-j` 仍 `compileOnly`、**未扩展 MySQL 范围**；未使用 `libraries` 键。
- 门禁反转（不是删断言）：`verifyRelocatedSnakeYaml` 改为校验「loader 声明与类一致 + 13 组外置包前缀一个不留 + 四个业务类仍引用原始包名 + `taboolibMainTask` 指纹」，`build/muz-release-audit.py` 同步反转并新增 `loader_class_present` / `loader_declared` / `business_references`。
- 测试：`MuzPluginLoaderTest`（10 项，含反射读生产解析器实际登记的依赖与仓库、Central 必须用镜像常量、TabooLib 仓库必在、作用域不得 test/provided、loader 只依赖 JDK/Paper/Aether、不得设 User-Agent）、`MuzPluginLoaderExternalizationContractTest`（4 项源码契约：无 relocate、`implementation(` 不得出现、坐标与 build 侧一致、主源码不得引用旧重定位包）、`PaperPluginDescriptorTest` 新增 loader 声明项、`MuzExternalDependencyProbe`（真实解析/缓存/加载探针，Gradle 任务 `muzExternalDependencyProbe`，需联网、不挂 `build`）。
- 验证（全部真实执行）：三目标串行 `clean shadowJar zipResourcePack zipCraftEngineBundle verifyRelocatedSnakeYaml` 各 **12 tasks executed** 成功；`build/muz-release-audit.py` **0 failures**（版本 1.10.54、作者仅 `linmumua`、folia=true、三目标外置包条目数全 0、loader 类与声明一致、业务引用 ok、计数 jar 833 / resourcepack 450 / craftengine 467）；`paper-26.2` 强制 `compileJava compileTestJava --rerun-tasks`（8 tasks executed）后全量 JUnit **1241 项 / 185 容器 / 1241 通过 / 0 失败 / 0 跳过**；探针冷缓存解析 **10 个 JAR / 17,716,303 字节**，`setOffline(true)` 离线重解析成功，SnakeYAML/Gson/sqlite（含 SPI）/reflex/ASM/commons-lang3/kotlin-stdlib 全部 ok，热缓存复跑零下载。
- JAR 体积对比：1.10.52 为 23,316,752 字节 / 3115 条目；1.10.54 为 5,594,749（1.21.11）/ 5,595,232（26.1.2）/ 5,595,186（26.2）字节、833 条目，**约 −76.0%**。`META-INF` 只剩 `MANIFEST.MF` 与 `META-INF/taboolib/*`（sqlite SPI、gson proguard、kotlin/reflex 模块文件都随库移出）。
- 门禁反证（在 `build/` 内对产物副本操作，不碰被版本控制的文件、不重建）：注入 `org/yaml/snakeyaml/Yaml.class` → 审计报「JAR 内仍含外置依赖 snakeyaml(原始)」；删除 `MuzPluginLoader.class` → 报「缺少插件加载器类」；让 `MuzYamlConfig.class` 不再引用原始 snakeyaml 包名 → 报「未引用原始包名」。三种变异都 exit 1，恢复后重跑仍 0 failures。另把加载器坐标版本临时改成 `2.7` 实测让两个 loader 测试变红（断言有效），恢复后 14/14 绿。
- **未验证 / 剩余风险（如实）**：本轮**未部署、未重启服务端、未做客户端验收**，没有任何实服运行期证据；「首次启动联网下载」「缓存 `<服务端根>/libraries/` 的离线边界」「下载失败必须是插件加载失败而不是半启动」「`folia-supported: true` 下的加载顺序」都只由探针 + 编译 + 单测支撑，必须由实服启动确认。外置方案另有固有耦合：依赖第三方 Maven 仓库可用性、缓存目录被清空或换机器需重新联网、管理员若把 Central 镜像指向不可达地址会表现为加载失败。**未改 `CLAUDE.md`**（按要求）。
