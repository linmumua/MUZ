...
- 版本号写在 `build.gradle.kts` 的 `version`，当前 `1.10.16`。`paper-plugin.yml` 用 `${version}` 占位，不要手写。
...
- `PackAssets`（`assets`）：插件侧复算资源包字形码位与字体名，零文件 IO。必须与 `build.gradle.kts` 生成逻辑严格对齐，两侧不一致会导致游戏内显示豆腐块；记牌器按牌类、矩形框、数字三层 glyph 和固定 15 格几何对齐，hotbar 底图（0xEF00）按 182×22 完整 9 槽对齐（槽 0..5 纯色、槽 6/7/8 图标+烘焙数字），选中框（`HOTBAR_SELECT_CODEPOINT` = 0xEF02）按 20×22、advance=21 对齐。
...
- `HudResourcePackVerifier`（`compat`）：异步从实际 ZIP 中央目录读取 HUD 白名单内容，核对解压长度/CRC、内置 SnakeYAML 声明的 font/char/file/height/ascent、三档各 22 张记牌器 PNG、hotbar PNG 和覆盖层；记牌器码位按 `scale + downTier×22 + layer` 对齐；拒绝旧字体分页、旧大号数字映射和原版 hotbar 覆盖。生成路径与 CE 上传源文件路径须一致或逐字节一致。服务端内容校验、远端上传完成、客户端应用是三件事，后两者无回执时保持未确认。
...
- 仍然禁止通过运行期配置修改构建期烘焙参数：`hotbar-hud.glyph-ascent`、`hotbar-hud.slot-count` 这两个键**运行期从未被读取**（纯文档键，记录构建期约定）。hotbar 默认 100% 底图字形为 **182×22**、advance 为 **183**，完整绘制 9 个 `18×20px` 槽，槽间距与左右外边距均为 `2px`，不画白色边框；槽 0..5 为纯色块，槽 6/7/8 烘焙物品图标 + 固定示例数字。运行期可选 75/100/125 三档真实资源，档位的图片、字体、advance、槽位与选中框几何由 `PackAssets` 和 `PreviewGeometry.hotbars` 同源提供，不能只改 CSS。选中框默认字形为 **20×22**、advance 为 **21**，四周 2px 亮黄（`#FFE040`）空心描边、中间透明；默认码位为底图 0xEF00、选中框 0xEF02，默认调试覆盖层为 0xEF01/0xEF03。尺寸、码位与 advance 必须同时修改 `build.gradle.kts` 和 `PackAssets`。要调垂直位置请用当前 scale 的 `offset-y` 覆盖层，不要改这些构建期约定。
...
- 三目标九个 1.10.16 产物已完成 5114 项资源审计：归档 CRC、目标 API/字节码、唯一作者、SnakeYAML relocation、125 个 OGG、三档 hotbar、三档记牌器、内嵌/独立 CraftEngine bundle 字节一致性及原版 hotbar 覆盖均通过；`paper-1.21.11` 与 `paper-26.1.2` 没有当次测试编译输出，未宣称其测试通过。三个 JAR 已复制到 `C:\PluginLibs`，副本 SHA-256 与构建产物一致。
