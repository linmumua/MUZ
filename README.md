...
> **当前事实（勿与下方历史记录混用）**：产物声明 `folia-supported: false`——迁移期间为采集实服证据临时改过的 `true` **已经回滚**。下方 64～74 行等历史段落中出现的「仍为临时 `true`」「守护测试按设计变红」「全量 JUnit 1123 / 1127 / 1171 项」等，都是**各自轮次的快照**，不代表当前状态；`folia-supported` 现为 `false`，守护测试应全绿，当前测试类数量以仓库实际测试目录为准（约 173 个）。TabooLib 已作为工具层实际接入并内嵌进 JAR。**关闭清理仍未完成**：`onDisable` 内对牌桌/麻将 owner 清理的注册会被调度门面拒绝，因此关闭期实际不会执行区域清理，不要据此宣称已生效。
...
> **本段末尾勘误（本轮，2026-09-24）**：上方 64～76 行各段的末尾普遍写着「`folia-supported` 仍为临时 `true`」「2 项失败为 `FoliaDeclarationMatchesSchedulerTest` 按设计变红」「全量 JUnit 1086 / 1097 / 1123 / 1127 / 1171 项」——这些都是**各轮次当时的快照**，**不是现状**。冒烟结束后 `folia-supported` **已恢复为 `false`**，`FoliaDeclarationMatchesSchedulerTest` 现应全绿（失败即代表 `false` 被误改回，属真实回归）；当前测试数量以仓库实际测试目录为准（约 170 个测试类）。**关闭清理仍未完成**（`onDisable` 内 owner 清理注册会被调度闸门拒绝）。TabooLib 已作为工具层实际接入。原文保留不改，仅此追加说明。
...
| TabooLib（`io.izzel.taboolib:common-reflex`） | 内置 | 反射等工具层；已重定位内嵌进插件 JAR，无需单独安装 |
| CraftEngine 0.0.67 | 可选 | 资源包与家具渲染 |
| PlaceholderAPI 2.12.2 | 可选 | 变量支持 |
| VaultAPI 1.7 | 可选 | 经济桥接 |
| CMI / EzEconomy / XConomy | 可选 | 经济 provider 检测 |

所有可选依赖缺失时自动降级，不会导致启动失败。TabooLib 仅作为工具库内嵌（不做注解生命周期），不需要作为插件安装。
