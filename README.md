...
1.10.57 变更（2026-09-26，升版本，**交互提示噪声修复**）：按用户提供的 1.10.55 实服日志做两项最小修复，均只碰交互提示路径。① 确认 1.10.56 已把「实体不存在」改为静默——1.10.55 那版 `PlayerOutputDispatcher` 用同一条 `reportCrossRegionSkip(..., "实体不存在")` 告警覆盖「实体不存在」与「跨 region」，1.10.56 起改为经 `EntityRegionLane` 投递到实体 owner region，`ENTITY_ABSENT`（提交时实体已不在）、执行时 `!entity.isValid()`、玩家离线三者都不留痕，只有四类异常留痕（实体中途换 region、投递被关闭期拒绝、投递接缝抛错、玩家可见性 API 抛错，按实体 30 秒限频）；本轮全主源码扫描确认已无该告警字符串，**未做改动**。② 新增 `linmumua.doudizhu.game.InteractionRejectionException`（继承 `IllegalStateException`），把**金币/筹码门槛不足**（`GameTable.addPlayer` / `toggleReady` / `ensureSeatEntryEligibility`、`PhysicalTableManager.joinSeat`）与系统故障分类：`PhysicalTableManager.reportInteractionFailure` 是**唯一**分类点，业务拒绝只保留玩家提示、不记堆栈；其它异常继续记录完整堆栈并限频。判据是**异常类型**而非消息文本；规则类拒绝（座位已有人、阶段不符）保持原样继续留痕。验证：`paper-26.2` `compileJava compileTestJava processResources processTestResources --rerun-tasks` 成功；定向 5 类 **26/26**；全量 JUnit **1260/1260、187 个容器**（0 失败/跳过/中止）；新增行为测试 `InteractionFailureLoggingBehaviorTest.金币门槛业务拒绝只提示不记堆栈而其它异常仍留痕`（同一个 JOIN 按钮两种结果）与源码契约 `InteractionFailureLoggingContractTest.门槛不足必须一律走业务拒绝类型而不是裸IllegalStateException`（钉住四个抛点），并做三次 mutation 反证（禁用分类 / 加入抛点退回裸 `IllegalStateException` / 开局资格抛点退回裸类型，各自命中对应用例，恢复后绿）。**未打包、未部署、未重启测试端，无实服运行期证据**；实服需重启后点一次余额不足的 JOIN 确认控制台不再出现该堆栈。

1.10.56 变更（2026-09-25，升版本，**用户可见行为修复**）：实体可见性改投**实体 owner region**。根因由 javap 反编译测试端 `folia-26.1.2.jar` 落实（不是推断）：`CraftEntity#getHandle()` 带 `TickThread.ensureTickThread(entity, "Accessing entity state off owning region's thread")` 门禁，而 `Player#showEntity/hideEntity` 的实现会读 `Entity#isVisibleByDefault()` 并调 `ChunkMap$TrackedEntity#updatePlayer`（内部按 `canSee` 决定是否把玩家加入 `seenBy`），所以这两个 API 只有在实体所属 region 上才合法；玩家侧那一半没有门禁（`CraftPlayer#getHandle()` 是裸字段读、`ServerGamePacketListenerImpl#send` 无门禁、`updatePlayer` 只有 `AsyncCatcher.catchOp`）。新增 `EntityRegionLane` 接缝 + 生产实现 `schedulingEntityRegionLane`（`Bukkit.getEntity` 查表 → `MuzScheduler.runEntity` 投递），执行体顺序为「实体有效性 → 归属复核 → 解析在线玩家 → 调玩家 API」，且只允许做这一件事。告警从「每玩家 × 每实体每 30 秒」降为「只在四类真异常时按实体限频」，实体不存在与玩家离线改为静默。验证：`paper-26.2` 强制 `compileJava compileTestJava` 成功；定向 40/40（`PlayerOutputDispatcherEntitySkipBehaviorTest` 10 项 + 新增契约 `PlayerOutputDispatcherEntityRegionContractTest` 4 项 + 既有 `PlayerOutputDispatcherTest`、`PhysicalTableOwnerTickContractTest`）。mutation 反证：`reportEntityLaneSkip` 早退 → 6/10 变红；投递退回 player lane → 8/10 变红（含两条核心断言），恢复后全绿。**未部署、未重启测试端、未进服**，跨 region 的实际客户端可见性仍需实服确认；同族残留（`TableSpeechPanelService#spawnPanel`、`TableGadgetEffectService#spawnDisplay`）本轮未动，细节见 AGENTS.md 的 1.10.56 段。

1.10.55 变更（2026-09-25，升版本，纯内部缺陷、无用户可见行为变化）：`CraftEngineFurnitureService` 的 CE 家具反射兼容性——方法查找恢复「父类/接口也查找」的继承链语义（CE 26.8 的 `furnitureById(Key)` 声明在父类与接口上，原先只认本类声明会把它误判为缺失），并把 TabooLib 抛出的受检 `NoSuchMethodException` 统一转成带「期望签名 + 实际同名方法 + 实际方法表」的运行时异常，使「CE 缺 API」在 `onEnable` 阶段降级并留日志、而不是让插件加载失败；`furnitureById` 缺失只降级牌型探测。新增包内可见取类接缝 `ceClass(...)`，本类因此不再是 `final`（与同包 `CraftEngineOffsetService` 一致，**勿改回 `final`**）。验证：`paper-26.2` 强制 `compileJava compileTestJava processResources processTestResources --rerun-tasks` 成功，全量 JUnit **1248/1248、186 个容器**通过（含新增 7 项测试与两次 mutation 反证）；**无实服运行期证据**，细节见 AGENTS.md 的 1.10.55 段。
