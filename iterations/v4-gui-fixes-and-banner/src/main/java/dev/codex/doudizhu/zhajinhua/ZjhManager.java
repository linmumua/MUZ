...
public final class ZjhManager {
    ...
    public ZjhTable createTable(String rawName, int maxPlayers) {
        ...
        if (key == null || key.isBlank()) {
            throw new IllegalArgumentException("炸金花牌桌名称不能为空。");
        }
        if (tables.containsKey(key)) {
            throw new IllegalArgumentException("已存在同名炸金花牌桌。");
        }
        ZjhTable table = new ZjhTable(plugin, this, rawName, maxPlayers);
    ...
    public ZjhTable joinTable(Player player, String rawName) {
        if (getTableOf(player) != null) {
            throw new IllegalArgumentException("你已经在一个炸金花牌桌里了。");
        }
        ZjhTable table = getTable(rawName);
        if (table == null) {
            throw new IllegalArgumentException("找不到这个炸金花牌桌。");
        }
    ...
    public void leaveTable(Player player) {
        ...
        if (table == null) {
            throw new IllegalArgumentException("你当前不在任何炸金花牌桌里。");
        }
        table.removePlayer(player, player.getName() + " 离开了炸金花牌桌。");
