...
- `build/libs/MUZ-1.3.1.jar`
- `build/libs/MUZ-1.3.1-dev.jar`
- `build/distributions/MUZ-resourcepack-1.3.1.zip`
- `build/distributions/MUZ-craftengine-1.3.1.zip`

推荐把重映射后的 `MUZ-1.3.1.jar` 放进服务端 `plugins/`。
如果你不用 CraftEngine，就给客户端下发 `MUZ-resourcepack-1.3.1.zip`。
如果你使用 CraftEngine，可以直接用 `MUZ-craftengine-1.3.1.zip`，或者让插件在检测到 CraftEngine 后自动把 bundle 导出到其数据目录。
